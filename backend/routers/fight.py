...
from metrics.unit_metric import AlphaStrike, BetaStrike, average_metric, plot_cdf
...
@router.post("/beta-strike")
def beta_strike(attacker: dict, defender: dict):
    result = {}
    attacker_profile = Profile(attacker)
    defender_profile = Profile(defender)
    ...
    metric = BetaStrike(ennemy_unit=defender_profile, return_n_slain_models=False, scale_by_cost=False)
    result["average_damage"] = average_metric(
        attacker_profile, metric, n_samples=5000, combat_context=attacker_context
    )
    img_buffer = plot_cdf(attacker_profile, metric, n_samples=5000, combat_context=attacker_context)
