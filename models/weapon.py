...
from rules.utils import bound_hit_wound_target_value, bound_save_target_value, roll_test, roll_test_with_crit
...
class Weapon:
    ...
    def _process_save_rolls(self, wounds: int, enemy_save: int, combat_context: dict) -> dict:
        ...
        rend = self.rend + self._find_modifier_total_value("rend", combat_context)
    ...
    def _process_ward_rolls(self, total_damage: int, enemy_ward: int) -> dict:
        """
    ...
    def attack(self, enemy_save: int, combat_context: list = None):
        """
        ...
        hit_rolls = self._process_hit_rolls(self.attacks, combat_context)
        ...
        if combat_context.get(["ward"]):
            total_damage = self._process_ward_rolls(total_damage, combat_context["ward"])["final_damage"]
